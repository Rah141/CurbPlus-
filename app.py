...
uploaded = st.file_uploader("Upload Liva Monthly Reports Excel", type=["xlsx", "xls", "xlsm"])

if uploaded:
    with st.spinner("Reading Excel…"):
        df = read_excel_sheet(uploaded, sheet_name="Liva Monthly Reports")

        missing = validate_columns(df)
        if missing:
            st.error("❌ **Column Validation Failed**")
            st.markdown("The following required columns are missing from your Excel file:")
            for col in missing:
                st.markdown(f"• `{col}`")
            st.markdown("Please ensure your Excel file contains all required columns.")
            st.stop()

        st.success("✅ **File uploaded and validated successfully!**")
        st.markdown("All required columns found. You can now search for policies.")
            ...
            if not query.strip():
                ...
                with st.spinner("🔍 Searching policies..."):
                    ...
                    if results.empty:
                        ...
                        # Show result cards above the table
                        st.write(f"Found {len(results)} matching policies:")
                        # Nice summary cards on top
    ...
    st.info("👆 **Please upload your Liva Monthly Reports Excel file to begin searching policies.**")
    st.markdown("""
    **Supported formats:** `.xlsx`, `.xls`, `.xlsm`  
    **Required sheet:** "Liva Monthly Reports"
    """)
